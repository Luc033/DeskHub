...
import { Copy, Trash2, Pencil, CheckCircle2, Smile, X, Save } from 'lucide-react';

export default function Emojis({ searchQuery, refreshKey }) {
  ...
  useEffect(() => {
    fetch(`/api/emojis`)
      ...
      .catch(err => console.error("Erro ao buscar emojis:", err));
  }, [refreshKey]);

  const handleDelete = async (id) => {
    if(window.confirm("Tem certeza que deseja excluir este emoji?")) {
      try {
        await fetch(`/api/emojis/${id}`, { method: 'DELETE' });
        setEmojis(emojis.filter(e => e.id !== id));
        showToast('Emoji excluído!');
      } catch (error) {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/emojis/${editingItem.id}`, {
        method: 'PUT',
      ...
      setEditingItem(null);
      showToast('Emoji atualizado com sucesso!');
    } catch (error) {
  ...
  return (
    <div className="animate-in fade-in duration-500 ">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-300 dark:bg-slate-800 dark:border dark:border-slate-700">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 dark:border-slate-700">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2 dark:text-slate-200">
                <Pencil className="h-5 w-5 text-emerald-600 dark:text-emerald-400" /> Editar Emoji
              </h2>
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5 dark:text-slate-300">Nome do Emoji / Atalho</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5 dark:text-slate-300">Valor ou Código Exato</label>
                <input required value={editingItem.value} onChange={e => setEditingItem({...editingItem, value: e.target.value})} className="w-full bg-slate-50 border border-slate-200 px-4 py-2.5 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm font-mono dark:bg-slate-900/50 dark:border-slate-700 dark:text-slate-200" />
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(e => (
          <div key={e.id} className="bg-white p-4 rounded-2xl border border-slate-200 hover:border-emerald-500/30 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between h-full dark:bg-slate-800 dark:border-slate-700 dark:hover:border-emerald-500/50">
            <div className="flex justify-between items-start mb-3">
              <span className="text-sm font-bold text-slate-800 truncate dark:text-slate-200">{e.name}</span>
              <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => setEditingItem(e)} className="p-1 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded transition-colors dark:hover:bg-slate-700/50 dark:hover:text-blue-400" title="Editar">
                  <Pencil className="h-4 w-4" />
                </button>
                <button onClick={() => handleDelete(e.id)} className="p-1 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors dark:hover:bg-red-950/30 dark:hover:text-red-400" title="Eliminar">
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
            </div>
            
            <div className="bg-slate-50 p-2 rounded-lg border border-slate-100 flex items-center justify-between gap-2 dark:bg-slate-900/50 dark:border-slate-700/50">
              <code className="text-xs text-slate-500 truncate dark:text-slate-400">{e.value}</code>
              <button onClick={() => handleCopy(e.value)} className="p-1.5 text-emerald-600 hover:bg-emerald-100 rounded transition-colors shrink-0 bg-emerald-50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:hover:bg-emerald-500/20" title="Copiar">
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 text-slate-400 bg-white border border-dashed border-slate-300 rounded-2xl mt-4 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-500">
          ...
          <p className="font-medium text-slate-500">Nenhum emoji encontrado.</p>
        </div>
      )}
    </div>
  );
